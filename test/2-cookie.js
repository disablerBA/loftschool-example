...
describe('ДЗ 7.2 - Cookie editor', () => {
    ...
    describe('Интеграционное тестирование', () => {
        ...
        it('cookie должны удаляться при нажатии на "удалить"', () => {
            ...
            deleteButton.click();
            cookies = getCookies();
            assert.equal(Object.keys(cookies).length, 0, `cookie не удалена из браузера ${Object.keys(cookies).length}`);
            assert.equal(listTable.children.length, 0, `cookie не удалена из таблицы ${listTable.children.length}`);
        });
